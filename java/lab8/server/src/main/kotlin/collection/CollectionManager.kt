...
class CollectionManager(private val databaseManager: DatabaseManager) {
    ...
    /**
     * Returns all elements sorted by default order as structured entries.
     *
     * Each entry carries the collection key, the login of the user who created the
     * product, and the product itself. This read operation works exclusively against
     * the in-memory collection.
     *
     * @return list of entries sorted by product price ascending
     */
    fun showStructured(): List<ProductEntry> {
        val snapshot = synchronized(products) { products.entries.toList() }
        return snapshot.stream()
            .sorted { a, b -> a.value.compareTo(b.value) }
            .map { (k, v) -> ProductEntry(k, creatorLogins[v.id] ?: "", v) }
            .toList()
    }
