def main():
    ...
    list_input_type = int(input(
        'Выберите способ формирования диапазона: 1 - границы, 2 - ручной ввод. Введите только цифру 1 или 2: '))
    ...
    alg_type = input(
        'Выберите алгоритм поиска: bin - бинарный поиск, seq - последовательный. Введите только слово bin или seq: ')
...
def guess_number(
    ...
    Returns:
        (tuple[int, int | None]): Пара целых чисел: искомое число и 
    ...
    if not hasattr(lst, '__iter__'):
        raise TypeError(
            f'Arg lst is \'{type(lst)}\', \'{type(lst)}\' object is non iterable.')
